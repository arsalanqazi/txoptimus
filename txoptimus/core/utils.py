...
def ensure_compatible_metadata(data_folder):
    """Auto-detect and regenerate node.csv/edges.csv if incompatible with kg.csv.
    
    This handles the case where kg.csv is from OptimusKG (ontology-prefixed IDs)
    but node.csv/edges.csv are still from PrimeKG (numeric IDs). When detected,
    it regenerates both files from kg.csv so the DataSplitter can work correctly.
    """
    kg_path = os.path.join(data_folder, 'kg.csv')
    node_path = os.path.join(data_folder, 'node.csv')
    
    if not os.path.exists(kg_path) or not os.path.exists(node_path):
        return  # Nothing to check
    
    # Quick compatibility check: try to map a few kg IDs to node IDs
    kg_sample = pd.read_csv(kg_path, nrows=100, low_memory=False)
    nodes = pd.read_csv(node_path, sep='\t', low_memory=False)
    
    id2idx = dict(zip(nodes['node_id'].astype(str), nodes['node_index']))
    sample_ids = kg_sample['x_id'].astype(str)
    match_rate = sample_ids.map(id2idx).notna().mean()
    
    if match_rate >= 0.5:
        print(f'node.csv/edges.csv compatible with kg.csv (match rate: {match_rate:.1%})')
        return
    
    print(f'Detected incompatible node.csv/edges.csv (match rate: {match_rate:.1%})')
    print('Regenerating metadata from kg.csv... this takes several minutes...')
    _regenerate_metadata(data_folder)


def _regenerate_metadata(data_folder):
    """Generate node.csv and edges.csv from kg.csv.
    
    Extracts all unique nodes from both sides of kg.csv, assigns integer indices,
    and creates properly formatted node.csv and edges.csv files that are
    compatible with the DataSplitter class.
    """
    import gc
    ...
    for f in [node_path, edges_path]:
        backup = f + '.primekg_backup'
        if os.path.exists(f) and not os.path.exists(backup):
            os.rename(f, backup)
            print(f'  Backed up {os.path.basename(f)}')
    
    # Read the full KG
    print('  Reading kg.csv...')
    kg = pd.read_csv(kg_path, low_memory=False)
    print(f'  KG: {len(kg)} edges')
    
    # Extract unique nodes from both x and y sides
    print('  Extracting unique nodes...')
    x_nodes = kg[['x_id', 'x_type', 'x_name']].drop_duplicates(subset=['x_id']).rename(
        columns={'x_id': 'node_id', 'x_type': 'node_type', 'x_name': 'node_name'})
    y_nodes = kg[['y_id', 'y_type', 'y_name']].drop_duplicates(subset=['y_id']).rename(
        columns={'y_id': 'node_id', 'y_type': 'node_type', 'y_name': 'node_name'})
    all_nodes = pd.concat([x_nodes, y_nodes]).drop_duplicates(subset=['node_id']).reset_index(drop=True)
    del x_nodes, y_nodes
    
    all_nodes['node_index'] = all_nodes.index
    all_nodes['node_source'] = all_nodes['node_type']  # default: use type as source
    
    print(f'  Unique nodes: {len(all_nodes)}')
    print(f'  Node types: {dict(all_nodes["node_type"].value_counts())}')
    
    # Save node.csv (tab-separated, matching PrimeKG format)
    all_nodes[['node_index', 'node_id', 'node_type', 'node_name', 'node_source']].to_csv(
        node_path, sep='\t', index=False)
    print(f'  Saved node.csv ({len(all_nodes)} nodes)')
    
    # Build edges.csv
    print('  Building edges.csv...')
    id2idx = dict(zip(all_nodes['node_id'].astype(str), all_nodes['node_index']))
    del all_nodes
    
    edges_x = kg['x_id'].astype(str).map(id2idx)
    edges_y = kg['y_id'].astype(str).map(id2idx)
    
    edges = pd.DataFrame({
        'relation': kg['relation'],
        'display_relation': kg['relation'],
        'x_index': edges_x,
        'y_index': edges_y
    })
    del edges_x, edges_y, kg
    
    # Drop any edges with unmapped nodes (should be 0, but just in case)
    unmapped = edges['x_index'].isna().sum() + edges['y_index'].isna().sum()
    if unmapped > 0:
        print(f'  WARNING: Dropping {unmapped} edges with unmapped nodes')
    ...
    print(f'  Saved edges.csv ({len(edges)} edges)')
...
def preprocess_kg(path, split, test_size = 0.05, one_hop = False, mask_ratio = 0.1):
    if split in ['cell_proliferation', 'mental_health', 'cardiovascular', 'anemia', 'adrenal_gland','autoimmune', 'metabolic_disorder', 'diabetes', 'neurodigenerative']:
        ...
        # Auto-detect and fix incompatible metadata files
        ensure_compatible_metadata(path)

        ds = DataSplitter(kg_path = path)

        test_edges = ds.get_test_kg_for_disease(name2id[split], test_size=test_size, one_hop=one_hop, mask_ratio=mask_ratio)
        df_test_edges = test_edges[['x_index', 'y_index']].copy()
        df_test_edges['x_index'] = df_test_edges['x_index'].astype(int)
        df_test_edges['y_index'] = df_test_edges['y_index'].astype(int)
        
        all_kg = ds.kg.copy()
        all_kg['x_index'] = all_kg['x_index'].astype(int)
        all_kg['y_index'] = all_kg['y_index'].astype(int)
        
        all_kg['split'] = 'train'
        all_kg = pd.merge(all_kg, df_test_edges, on=['x_index', 'y_index'], how='left', indicator=True)
        all_kg.loc[all_kg['_merge'] == 'both', 'split'] = 'test'
        all_kg = all_kg.drop('_merge', axis=1)
        ...
        df = df[['x_type', 'x_id', 'relation', 'y_type', 'y_id', 'split']].copy()
    ...
    # Drop any rows where relation is NaN (can occur from right-join merge mismatches)
    df = df.dropna(subset=['relation']).reset_index(drop=True)
    ...
    for i in tqdm(unique_relation):
        if ('_' in i) and (i.split('_')[0] == i.split('_')[1]):
            # homogeneous graph, remove reverse duplicates
            df_temp = df[df.relation == i]
            ids = df_temp[['x_id', 'y_id']].astype(str).values
            ids.sort(axis=1)
            temp_df = pd.DataFrame(ids, index=df_temp.index)
            dups = temp_df[temp_df.duplicated()].index
            to_drop.append(dups)
            
    if len(to_drop) > 0:
        to_drop = pd.Index(np.concatenate(to_drop))
    ...
    x_idx_vals = np.zeros(len(df))
    y_idx_vals = np.zeros(len(df))

    idx_map = {}
    print('Iterating over node types...')
    for i in tqdm(unique_node_types):
        x_mask = (df.x_type == i).values
        y_mask = (df.y_type == i).values
        names = pd.unique(np.concatenate([df.x_id.values[x_mask], df.y_id.values[y_mask]]))
        names2idx = {name: idx for idx, name in enumerate(names)}
        
        if x_mask.any():
            x_idx_vals[x_mask] = df.x_id.iloc[x_mask].map(names2idx).values
        if y_mask.any():
            y_idx_vals[y_mask] = df.y_id.iloc[y_mask].map(names2idx).values
        idx_map[i] = names2idx
    ...
    import gc
    del x_idx_vals, y_idx_vals
    gc.collect()
    df.to_csv(os.path.join(path, 'kg_directed.csv'), index = False, chunksize=100000)
...
def random_fold(df, fold_seed, frac):
    ...
    train_res = pd.concat([x for x in train_chunks if not x.empty]).reset_index(drop=True) if any(not x.empty for x in train_chunks) else pd.DataFrame(columns=df.columns)
    val_res = pd.concat([x for x in val_chunks if not x.empty]).reset_index(drop=True) if any(not x.empty for x in val_chunks) else pd.DataFrame(columns=df.columns)
    test_res = pd.concat([x for x in test_chunks if not x.empty]).reset_index(drop=True) if any(not x.empty for x in test_chunks) else pd.DataFrame(columns=df.columns)
...
def complex_disease_fold(df, fold_seed, frac):
    ...
    df_train = pd.concat(train_chunks + [df_dd_train])
    df_valid = pd.concat(val_chunks + [df_dd_valid])
    df_test = pd.concat(test_chunks + [df_dd_test])
...
class Full_Graph_NegSampler:
    ...
    def __call__(self, graph):
        ...
        # Check if the negative graph is entirely empty
        if len(out) == 0:
            print("Warning: No negative edges generated. Returning empty graph.")
            out = {etype: (torch.tensor([], dtype=torch.int64, device=self.device), torch.tensor([], dtype=torch.int64, device=self.device)) for etype in graph.canonical_etypes}
...
def evaluate_graph_construct(df_valid, g, neg_sampler, k, device):
    ...
    for etype in g.canonical_etypes:
        try:
            ...
            if len(df_temp) == 0:
                src = torch.tensor([], dtype=torch.int64, device=device)
                dst = torch.tensor([], dtype=torch.int64, device=device)
            else:
            ...
            print(f"Failed to process relation {etype[1]}: {e}")
            
    # Check if the entire graph is empty
    has_edges = any(len(edges[0]) > 0 for edges in out.values())
    if not has_edges:
        print("Warning: Entire graph is empty. Returning None, None.")
        return None, None
...
def evaluate_fb(model, g_pos, g_neg, G, dd_etypes, device, return_embed = False, mode = 'valid'):
    ...
    # Only concatenate scores for edge types that have predictions.
    # Test/validation graphs may be missing edge types (e.g. contraindication)
    # if the data split has zero edges for that relation.
    pos_etypes_present = [i for i in dd_etypes if i in pred_score_pos]
    neg_etypes_present = [i for i in dd_etypes if i in pred_score_neg]
    
    pos_score = torch.cat([pred_score_pos[i] for i in pos_etypes_present]) if pos_etypes_present else torch.tensor([])
    neg_score = torch.cat([pred_score_neg[i] for i in neg_etypes_present]) if neg_etypes_present else torch.tensor([])
...
def evaluate_graphmask(model, G, g_valid_pos, g_valid_neg, only_relation, epoch, etypes_train, allowance, penalty_scaling, device, mode = 'validation', weight_bias_track = False, wandb = None, no_base = False):
    ...
    with torch.no_grad():
        ...
        # Guard: only include edge types present in the prediction output
        etypes_present = [et for et in etypes_train if et in original_predictions_pos]
        pos_score = torch.cat([original_predictions_pos[i] for i in etypes_present])
        neg_score = torch.cat([original_predictions_neg[i] for i in etypes_present])
        ...
        etypes_present_upd = [et for et in etypes_train if et in updated_predictions_pos]
        pos_score = torch.cat([updated_predictions_pos[i] for i in etypes_present_upd])
        neg_score = torch.cat([updated_predictions_neg[i] for i in etypes_present_upd])
        updated_predictions = torch.sigmoid(torch.cat((pos_score, neg_score)))
...
def obtain_protein_random_walk_profile(disease, num_walks, path_len, g, disease_etypes, disease_nodes, walk_mode):
    ...
    for _ in range(num_walks):
        successor = g.successors(disease, etype = 'disease_ASSOCIATED_WITH_gene/protein')
        if len(successor) > 0:
        ...
        for path_idx in range(path_len):
            successor = g.successors(current, etype = 'gene/protein_INTERACTS_WITH_gene/protein')
            if len(successor) > 0:
                current = choice(successor)
...
def process_disease_area_split(data_folder, df, df_test, split):
    ...
    for i,j in id2idx.items():
        ...
        if '_' in i_str:
            parts = i_str.split('_')
            # Check if it is a PrimeKG-style numeric merge (e.g. '123_456')
            if all(p.replace('.', '', 1).isdigit() for p in parts):
                for x in parts:
                    temp_dict[str(float(x))] = j
            else:
                # It's a prefixed ID like 'DOID_0050890'
                temp_dict[i_str] = j
        else:
            try:
                temp_dict[str(float(i_str))] = j
            except ValueError:
                temp_dict[i_str] = j
...
def disease_centric_evaluation(df, df_train, df_valid, df_test, data_path, G, model, device, disease_ids = None, relation = None, weight_bias_track = False, wandb = None, show_plot = False, verbose = False, return_raw = False, simulate_random = True, only_prediction = False):
    ...
    # Only read columns needed for name lookups (avoids loading all 21.8M rows)
    ...
    df_dd = df_[df_.x_type.isin(['drug', 'disease']) | df_.y_type.isin(['drug', 'disease'])]
    del df_  # free memory

    id2name_disease = dict(df_dd[df_dd.x_type == 'disease'][['x_id', 'x_name']].drop_duplicates().values)
    id2name_disease.update(dict(df_dd[df_dd.y_type == 'disease'][['y_id', 'y_name']].drop_duplicates().values))

    id2name_drug = dict(df_dd[df_dd.x_type == 'drug'][['x_id', 'x_name']].drop_duplicates().values)
    id2name_drug.update(dict(df_dd[df_dd.y_type == 'drug'][['y_id', 'y_name']].drop_duplicates().values))
    del df_dd  # free memory
