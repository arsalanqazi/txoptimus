...
class DistMultPredictor(nn.Module):
    def __init__(self, n_hid, w_rels, G, rel2idx, proto, proto_num, sim_measure, bert_measure, agg_measure, num_walks, walk_mode, path_length, split, data_folder, exp_lambda, device):
        ...
        if proto:
            ...
            is_optimus = ('disease', 'disease_PARENT_disease', 'disease') in G.canonical_etypes
            if is_optimus:
                self.disease_etypes_all = ['disease_PARENT_disease', 'disease_ASSOCIATED_WITH_gene/protein', 'disease_PHENOTYPE_PRESENT_effect/phenotype', 'rev_exposure_LINKED_TO_disease']
                self.disease_nodes_all = ['disease', 'gene/protein', 'effect/phenotype', 'exposure']
                self.disease_etypes = ['disease_PARENT_disease', 'disease_ASSOCIATED_WITH_gene/protein']
                self.disease_nodes = ['disease', 'gene/protein']
                self.protein_etypes = ['disease_ASSOCIATED_WITH_gene/protein']
            else:
                self.disease_etypes_all = ['disease_disease', 'rev_disease_protein', 'disease_phenotype_positive', 'rev_exposure_disease']
                self.disease_nodes_all = ['disease', 'gene/protein', 'effect/phenotype', 'exposure']
                self.disease_etypes = ['disease_disease', 'rev_disease_protein']
                self.disease_nodes = ['disease', 'gene/protein']
                self.protein_etypes = ['rev_disease_protein']
            
            disease_etypes = self.disease_etypes
            disease_nodes = self.disease_nodes
            disease_etypes_all = self.disease_etypes_all
            disease_nodes_all = self.disease_nodes_all
            ...
            for etype in self.etypes_dd:
                ...
                if sim_measure == 'all_nodes_profile':
                    ...
                    diseases_profile = {i.item(): obtain_disease_profile(G, i, self.protein_etypes, ['gene/protein']) for i in all_disease_ids}
    ...
    def forward(self, graph, G, h, pretrain_mode, mode, block = None, only_relation = None):
        with graph.local_scope():
            ...
            if len(graph.canonical_etypes) == 1:
                ...
                # Filter to only edge types present in the evaluation graph.
                # Test/validation graphs may be missing edge types (e.g. contraindication)
                # if the data split has zero edges for that relation.
                graph_etypes = set(graph.canonical_etypes)
                etypes_train = [et for et in self.etypes_dd if et in graph_etypes]
                                ...
                                for i in h_disease['disease_query_id'][0]:
                                    if i.item() not in self.diseases_profile_etypes[etype]:
                                        if self.sim_measure == 'all_nodes_profile':
                                            ...
                                            self.diseases_profile_etypes[etype][i.item()] = obtain_disease_profile(G, i, self.protein_etypes, ['gene/protein'])
