...
class TxGNN:
    ...
    def model_initialize(self, n_hid = 128, 
        ...
        if self.df_valid is not None and len(self.df_valid) > 0:
            ...
            if self.g_valid_pos is None:
                self.g_valid_pos, self.g_valid_neg = None, None
        else:
            self.g_valid_pos, self.g_valid_neg = None, None

        if self.df_test is not None and len(self.df_test) > 0:
            self.g_test_pos, self.g_test_neg = evaluate_graph_construct(self.df_test, self.G, 'fix_dst', 1, self.device)
            if self.g_test_pos is None:
                self.g_test_pos, self.g_test_neg = None, None
        else:
            self.g_test_pos, self.g_test_neg = None, None
    ...
    def pretrain(self, n_epoch = 1, learning_rate = 1e-3, batch_size = 1024, train_print_per_n = 20, sweep_wandb = None):
        ...
        # Wrap the sampler to handle edge prediction and negative sampling
        ...
        for epoch in range(n_epoch):
            ...
            if (epoch) % valid_per_n == 0:
                ...
                if best_val_acc < macro_auroc:
                    ...
                    self.best_model = copy.deepcopy(self.model)
                    # Save best model checkpoint immediately to prevent loss
                    # if a later phase (e.g. testing evaluation) crashes.
                    if hasattr(self, '_checkpoint_dir') and self._checkpoint_dir:
                        self.save_model(self._checkpoint_dir)
                        print(f'  >> Best model saved to {self._checkpoint_dir} (Macro AUROC: {best_val_acc:.4f})')
    ...
    def train_graphmask(self, relation = 'indication',
        ...
        for layer in reversed(list(range(self.graphmask_model.count_layers()))):
            ...
            for epoch in range(epochs_per_layer):
                ...
                # Aggressive GC every 10 epochs to prevent OOM from neg_sampler leak
                if epoch % 10 == 0:
                    import gc
                    gc.collect()
                ...
                if epoch % valid_per_n == 0:
                    ...
                    if loss_sum < best_loss_sum:
                        ...
                        self.best_graphmask_model = copy.deepcopy(self.graphmask_model)
                        # Continuous disk save so progress survives OOM kill
                        if hasattr(self, '_checkpoint_dir') and self._checkpoint_dir:
                            try:
                                torch.save(self.best_graphmask_model.state_dict(),
                                          os.path.join(self._checkpoint_dir, 'graphmask_model.pt'))
                                print(f">> Best GraphMask model saved to {self._checkpoint_dir} (loss={best_loss_sum:.4f})")
    ...
    def retrieve_save_gates(self, path, relation):
        whole_graph, _ = self.retrieve_gates_scores_penalties(relation)
