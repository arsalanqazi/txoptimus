...
from .utils import preprocess_kg, create_split, process_disease_area_split, create_dgl_graph, evaluate_graph_construct, convert2str, data_download_wrapper, reverse_rel_generation
...
class TxData:
    ...
    def prepare_split(self, split = 'complex_disease',
        ...
        if not os.path.exists(os.path.join(split_data_path, 'train.csv')):
            ...
            # Safeguard: if loaded splits are missing rev_ edges, generate them
            if not any(df_train.relation.str.startswith('rev_')):
                print('Safeguard: generating missing rev_ relations for loaded splits...')
    ...
    def retrieve_id_mapping(self):
        ...
        # Only read columns needed for name lookups, and filter to drug/disease rows
        # This avoids loading all 21.8M rows into memory (~6GB)
        df_ = pd.read_csv(os.path.join(self.data_folder, 'kg.csv'),
                          usecols=['x_id', 'x_type', 'x_name', 'y_id', 'y_type', 'y_name'])
        df_dd = df_[df_.x_type.isin(['drug', 'disease']) | df_.y_type.isin(['drug', 'disease'])].copy()
        
        df_dd['x_id'] = df_dd['x_id'].apply(lambda x: convert2str(x))
        df_dd['y_id'] = df_dd['y_id'].apply(lambda x: convert2str(x))

        id2name_disease = dict(zip(df_dd.loc[df_dd.x_type == 'disease', 'x_id'], 
                                    df_dd.loc[df_dd.x_type == 'disease', 'x_name']))
        id2name_disease.update(dict(zip(df_dd.loc[df_dd.y_type == 'disease', 'y_id'],
                                         df_dd.loc[df_dd.y_type == 'disease', 'y_name'])))

        id2name_drug = dict(zip(df_dd.loc[df_dd.x_type == 'drug', 'x_id'],
                                 df_dd.loc[df_dd.x_type == 'drug', 'x_name']))
        id2name_drug.update(dict(zip(df_dd.loc[df_dd.y_type == 'drug', 'y_id'],
                                       df_dd.loc[df_dd.y_type == 'drug', 'y_name'])))
